    ...
    useEffect(() => {
        Promise.all([getCardById(cardId), getAllUsers()])
            .then(([cardResult, usersArrayResult]) => {
                ...
                setOwner((usersArrayResult.data.find((user: User) => user.id == cardUserId)));
            })
        ...
        <>
            <Modal className={theme} show={show} onHide={() => onHide(false)}
                ...
                <Modal.Body className="modalTheme">
                    <img className="modalImage" src={card?.imageUrl} alt={card?.imageAlt} style={{ width: "100%", height: "30vh" }} />
                    ...
                    {card?.state && <p><b>State:</b> {card.state}</p>}
                    <p><b>Adress:</b> {card?.country}, {card?.city}, {card?.street} {card?.houseNumber}</p>
                    {card?.zip && <p><b>Zip:</b> {card.zip}</p>}
                </Modal.Body>
            ...
            <Modal className={theme} show={openOwnerModal} onHide={() => setOpenOwnerModal(false)}
                ...
                centered>
                <Modal.Header className="modalTheme" closeButton>
                    <Modal.Title>{owner?.firstName} {owner?.lastName}</Modal.Title>
                </Modal.Header>
                <Modal.Body className="modalTheme">
                    <div className="row">
                        <div className="col-md-8">
                            ...
                            <p className="mt-3"><b>Adress:</b> {owner?.country}, {owner?.city}, {owner?.street} {owner?.houseNumber}</p>
                        </div>
                        <div className="col-md-4">
                            <img src={owner?.imageUrl ? owner.imageUrl : owner?.gender == "Male" ? "male-profile.jpg" : "female-profile.jpg"} alt={owner?.imageAlt} style={{ width: "100%" }} />
                        </div>
                    </div>
                </Modal.Body>
                <Modal.Footer className="modalTheme">
